...
ABoard::ABoard()
{
    ...
    //Array para controlar que solo exista un board
    TArray<AActor*> cantidad;
    UGameplayStatics::GetAllActorsOfClass(GetWorld(), ABoard::StaticClass(), cantidad);
    if (cantidad.Num() > 1)
    {
        BoardInstance = Cast<ABoard>(cantidad[0]);
        //GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Yellow, FString::Printf(TEXT("%s Ya existe el board"), *BoardInstance->GetName()));
        Destroy();
    }
}
